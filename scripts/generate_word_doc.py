...
def create_doc(path, boot_S, p_S, boot_CI, p_CI, auc_S, auc_CI,
               motion, atlas, repl, mc):
    doc = Document()
    doc.add_heading("Parameter Sensitivity and Pilot Simulation of Synergy Metrics", level=1)

    doc.add_paragraph(
        f"ΔS={auc_S:.3f} (95% CI [{boot_S[0]:.3f}–{boot_S[1]:.3f}], p_perm={p_S:.3f}); "
        f"ΔCI={auc_CI:.3f} (95% CI [{boot_CI[0]:.3f}–{boot_CI[1]:.3f}], p_perm={p_CI:.3f})."
    )
    doc.add_paragraph(
