...
def main(out_dir):
    ...
    df = compute_synergy_ci(str(out/'cambridge'/'preprocessed'),
                            atlas='schaefer400',
                            thetas=thetas,
    ...
    auc_S,p_S = permutation_test_auc(df,'S')
    auc_CI,p_CI = permutation_test_auc(df,'CI')

    log.info("5/9 Motion model")
    motion = motion_covariate_analysis(df, str(out/'cambridge'/'preprocessed'))

    log.info("6/9 Atlas robustness")
    atlas_res = atlas_check(str(out/'cambridge'/'preprocessed'),
                             ('aal90','shen268'))

    log.info("7/9 Replication (Melbourne)")
    repl = run_replication(data_root='data/melbourne',
    ...
    create_doc(path=str(out/REPORT_DOC),
